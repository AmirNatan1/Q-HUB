...
| Partner Field | `[data-partner-state]` advances through `opening`, `strategic`, and `founding`; root `[data-partner-focus]` selects the sole visible organization from the semantic order carried by `[data-partner-sequence]`. | Quantum-origin space becomes a serious industrial ecosystem. Each organization owns the complete viewport through continuous material, crop, edge behavior, and identity scale rather than entering as an equal tile or a discrete branded panel. |
| Field Crossing | `[data-crossing-state]` advances through `outside`, `threshold`, and `field`. | One large round possibility is compressed between opposing constraint plates and resolves as a long, sharp rectilinear field probe while the directed material region gains authority. Motion changes morphology and material, not only position or color; repeating grids are absent. |
| Compressed FIND → TEST → PROVE | `[data-method-state]` advances through `find`, `test`, and `prove`. | FIND distributes paths and points toward selection; TEST introduces a hard vertical threshold and contact deformation; PROVE aligns the same residual observations against a common evidence plane. Every pair changes non-color geometry, and PROVE avoids checkmark/form/completion UI. |
| Activity progression | `[data-activity-state]` advances through `field-testing`, `programs`, `partner-engagement`, and `global-ecosystem`; the matching `[data-activity-signal]` and `[data-activity-geometry]` are exposed. | Each local scroll quarter pairs one contained category signal with a unique state-driven spatial condition: traversal/contact, staged cadence, shared-field alignment, or expanding reach. All four are reached without overlap, crop, continuous loops, or an invented activity feed. |
